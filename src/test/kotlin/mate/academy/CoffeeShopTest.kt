...
class CoffeeShopTest {
    ...
    fun `sell coffee with common client name and coffee type`() {
        ...
        assertEquals("Serving a Espresso to Alice. Served: true${System.lineSeparator()}", outputStream.toString())
    }

    @Test
    fun `sell coffee with empty client name`() {
        coffeeShop.sellCoffee("", "Latte")
        assertEquals("Serving a Latte to . Served: true${System.lineSeparator()}", outputStream.toString())
    }

    @Test
    fun `sell coffee with empty coffee type`() {
        coffeeShop.sellCoffee("Bob", "")
        assertEquals("Serving a  to Bob. Served: true${System.lineSeparator()}", outputStream.toString())
    }

    @Test
    fun `sell coffee with whitespace-only client name`() {
        coffeeShop.sellCoffee("   ", "Cappuccino")
        assertEquals("Serving a Cappuccino to    . Served: true${System.lineSeparator()}", outputStream.toString())
    }
    ...
    fun `re-sell coffee to same client`() {
        ...
        coffeeShop.sellCoffee("Eve", "Flat White")
        assertEquals(
            "Serving a Flat White to Eve. Served: true${System.lineSeparator()}" +
                    "Serving a Flat White to Eve. Served: true${System.lineSeparator()}",
